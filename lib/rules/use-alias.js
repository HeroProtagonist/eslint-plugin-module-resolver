  ...
  create: function (context) {
    ...
    if (Array.isArray(moduleResolverSettings.root)) { // get aliases based on root array
      ...
      const rootSubDirectories = rootAbsolutePaths
        .map(({ rootPath, root }) => ({ rootPath, root, directories: fs.readdirSync(rootPath, { withFileTypes: true }) })) // create pairs root paths and it's content
        .map(({ rootPath, root, directories }) => ({ // select only directories from path's content
          rootPath,
          root,
        ...
        .map(({ rootPath, root, directories }) => directories.map((directory) => ({
    ...
    const getAvaliableFix = ({ value, range, raw }) => {
      ...
      if (matchingAlias !== undefined && pathExists && value.match(/\.\.\//)) { // matches, exists, and starts with ../ 
