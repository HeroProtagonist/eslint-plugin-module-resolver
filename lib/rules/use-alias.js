...
module.exports = {
  ...
  create: function(context) {
    ...
    const hasError = val => {
      ...
      let pathExists = fs.existsSync(`${resolvedPath}${resolvedExt}`) || fs.existsSync(`${resolvedPath}/index.js`)

      if (extensions && !pathExists) {
        pathExists = extensions.filter(ext => fs.existsSync(`${resolvedPath}${ext}`)).length
      }
